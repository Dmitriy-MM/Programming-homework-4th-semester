...
List_2t<Database_link_node*> * Database_command::apply (Database& database, int& count, int do_print)
{
	...
	switch (type)
	{
		...
		case Command_type::select:
			{
				AVLt_tree<Database_link_node*> ** avl_name_trees = database.get_find_name_struct ();
				AVLt_tree<Database_link_node*> ** avl_phone_trees = database.get_find_phone_struct ();
				selected_list = new List_2t<Database_link_node*>;
				if ( // Нет возможности использовать быстрый поиск (а также поиск "name or phone").
					(op == Operation::lor) ||
					((
				...
				{
					if (c_phone == Condition::eq) // Изначально надо предпочитать поиск по имени, так как у него проще ф-я сравнения.
					{
						int index = Database::func_hash_phone (this->get_phone(), database.get_hash_p ());
						selected_list->join (apply_to_avl_subtree (c_phone, avl_phone_trees[index]->get_root (), avl_phone_trees[index]->get_compare_func (), avl_phone_trees[index]->get_compare_arg ()));
					}
					else if (c_name == Condition::eq)
					{
						int index = Database::func_hash_name (this->get_name(), database.get_hash_k ());
						selected_list->join (apply_to_avl_subtree (c_name, avl_name_trees[index]->get_root (), avl_name_trees[index]->get_compare_func (), avl_name_trees[index]->get_compare_arg ()));
					}
					else if ((c_phone != Condition::none) && (c_name != Condition::ne)) // Доступен поиск по телефону.
					{
						int p = Database::calc_dimension_hash_p (database.get_hash_p ());
						for (i = 0; i < p; i++)
						{
							// Поиск по i-му дереву.
							selected_list->join (apply_to_avl_subtree (c_phone, avl_phone_trees[i]->get_root (), avl_phone_trees[i]->get_compare_func (), avl_phone_trees[i]->get_compare_arg ()));
						}
					}
					else // Доступен поиск по имени.
					{
						int k = database.get_hash_k ();
						for (i = 0; i < k; i++)
						{
							// Поиск по i-му дереву.
							selected_list->join (apply_to_avl_subtree (c_name, avl_name_trees[i]->get_root (), avl_name_trees[i]->get_compare_func (), avl_name_trees[i]->get_compare_arg ()));
						}
					}
				}
...
List_2t_node<Database_link_node*> * Database_command::choosing (Condition condition, AVLt_tree_node<Database_link_node*> * sub_head, int (*cmp) (Database_link_node*, Database_link_node*, void *), void * cmp_arg)
{
	...
	tmp_list.join (tmp);
	tmp_list.join (apply_to_avl_subtree (condition, sub_head->get_left (), cmp, cmp_arg));
	tmp_list.join (apply_to_avl_subtree (condition, sub_head->get_right (), cmp, cmp_arg));
...
List_2t_node<Database_link_node*> * Database_command::apply_to_avl_subtree (Condition condition, AVLt_tree_node<Database_link_node*> * sub_head, int (*cmp) (Database_link_node*, Database_link_node*, void *), void * cmp_arg)
{
	int cmp_res;
	...
	switch (condition)
	{
		case Condition::eq:
			if (cmp_res < 0) { // this_ptr < *sub_head.
				return apply_to_avl_subtree (condition, sub_head->get_left (), cmp, cmp_arg);
			} else if (cmp_res > 0) {
				return apply_to_avl_subtree (condition, sub_head->get_right (), cmp, cmp_arg);
			} else {
				List_2t_node<Database_link_node*> * tmp = nullptr, * tmp2 = nullptr;
				for (AVLt_tree_node<Database_link_node*> * cur = sub_head; cur != nullptr; cur = cur->get_next ())
				{
		...
		case Condition::gt:
			if (cmp_res < 0) { // this_ptr < *sub_head.
				return choosing (condition, sub_head, cmp, cmp_arg);
			} else {
				return apply_to_avl_subtree (condition, sub_head->get_right (), cmp, cmp_arg);				
			}
		case Condition::ge:
